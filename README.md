...
```
直接引入`import ImageViewer from 'image-viewer-gallery'`即可。
...
```
注意，如果需要渐变打开的动画，不仅仅要提供`fadeInFn`函数，还要提供每一张图片的宽高尺寸数据(字段名为`w`和`h`)，如果同时提供`thumbnail`和`url`字段，则尺寸数据是相对`thumbnail`缩略图来说的，否则就是`url`大图的尺寸。
